...
restrain the developers. Any files that can be generated byt the application
...
The `src` folder contains only (1) actual application code, (2) static or
dynamic assets and (3) testing/helper functions to be used by the application.
Inside the `src` folder, you'll find 3 folders that contain these types of
code/information respectively. They are (1) `app`, (2) `static` and (3)
`testing`.

## Code

### Structure
Any module or submodule is contained inside its own folder (example:
`src/app/home` is its own component). No other information, asset, module or
...
Any component or module that is a direct child of the `app` folder is a starting
point for the application (this boilerplate has only one - `src/app/index.js`).
...
Also note that since this project is only a boilerplate and contains only one
component, there is no folder for containers/components.
...
Unit tests can generally be divided in 2 categories:
  - whitebox - for testing a service/module
  - blackbox - for testing a visual component of a web page
...
said, the tests focus only on testing the output on the page. This is done for
one main reason: changing the library/framework does not majorly impact the
tests (check the different _flavours_ of this boilerplate to actually see the
benefits).
